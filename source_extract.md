...
```json
{
  "compilerOptions": {
    ...
    "typeRoots": [
      "./node_modules/@types",
      "./src/types" // Ensure this path points to the directory containing your 'express' folder with index.d.ts
    ],
...
```json
{
  "name": "opensource-tms-backend",
  "version": "1.0.0",
  "description": "Open Source Transportation Management System Backend",
  "main": "dist/index.js",
  "scripts": {
...
## `backend/src/services/laneRateService.ts`
```ts
...
## `backend/src/services/integrations/saferService.ts`
```ts
...
## `backend/src/services/ai/emailService.ts`
```ts
...
## `backend/src/routes/carriers.ts`
```ts
import { Router } from 'express';
...
## `backend/src/models/Shipment.ts`
```ts
...
## `backend/src/models/EquipmentType.ts`
```ts
import mongoose, { Schema, Document } from 'mongoose';
...
## `backend/src/models/AccessorialType.ts`
```ts
...
## `backend/src/models/LaneRate.ts`
```ts
...
## `backend/src/controllers/accessorialTypeController.ts`
```ts
...
## `backend/src/controllers/shipperController.ts`
```ts
...
## `backend/src/controllers/laneRateController.ts`
```ts
